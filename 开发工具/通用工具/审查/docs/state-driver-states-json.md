# T1 输出格式 · states.json 汇总 <!-- nav -->

> 旧称：T1 `AuditStateDriver` / T3 `AuditTurntable` 等（原 `审查/README.md`） §4「states.json」前段。§ 编号沿用原文，跨页 § 引用按 [文档目录](../README.md#文档目录) 查。<!-- nav -->
> 上一页：[T1 输出格式 · `state_<id>.json`](state-driver-output.md) · 下一页：[T1 输出格式 · states.json 字段口径](state-driver-states-fields.md) <!-- nav -->
...
```json
{
  ...
  "volatile_probe": true,
  "tool_version": { "source_hash": "<64 位 sha256>", "deployed_files": 12,
                    "compile_time_utc": "2026-09-19T01:20:00Z",
                    "version_file": "Assets/AvatarAudit/VERSION",
                    "version_file_hash": "<同上>", "version_file_time_utc": "2026-09-19T01:19:00Z",
